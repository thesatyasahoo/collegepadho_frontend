...
const AddNewBlog = () => {
  ...
  const getApi = () => {
    axios.get(`${BASE_URL}/api/stream`).then((res) => { setStreamApi(res.data.response) })
  }
  ...
  const handleSend = () => {
    ...
    formData.append('content', JSON.stringify(descriptionState))

    axios.post(`${BASE_URL}/api/blog`, formData).then((res) => { alert(res.data.message); setIsSent(true) }).catch((err) => { console.log(err) })
    // console.log(blogTitle)
    // console.log(JSON.stringify(category))
    // console.log(shortDescription)
    // console.log(mainTitle)
    // console.log(selectedImage)
    // console.log(JSON.stringify(descriptionState))

  }
  useEffect(() => { if (isSent) { window.location.reload(); window.scrollTo(0, 0) } }, [isSent])
      ...
      <Box sx={{ ml: { lg: drawerWidth, md: drawerWidth } }}>
        <Box component="main" sx={{ bgcolor: "#f2f3f8", p: 3, width: "100%" }}>
          ...
          <Grid container sx={{ backgroundColor: 'white', p: "10px" }} spacing={1}>
            ...
            <Grid item xs={12} sm={12} md={12} lg={12} justifyContent={"left"}>
              <Typography color="#646c9a" fontSize="1rem" sx={{ mt: "5px", mr: "15px", textAlign: "left" }}>Blog title</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={12}>
              <OutlinedInput placeholder='Provide blog title' sx={{ height: '40px' }} fullWidth value={blogTitle} onChange={(e) => { setBlogTitle(e.target.value) }} />
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={12} justifyContent={"left"}>
              <Typography color="#646c9a" fontSize="1rem" sx={{ mt: "5px", mr: "15px", textAlign: "left" }}>Main title</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={12}>
              <OutlinedInput placeholder='Main title' sx={{ height: '40px' }} fullWidth value={mainTitle} onChange={(e) => { setMainTitle(e.target.value) }} />
            </Grid>

            <Grid item xs={12} sm={12} md={12} lg={12} justifyContent={"left"}>
              <Typography color="#646c9a" fontSize="1rem" sx={{ mt: "5px", mr: "15px", textAlign: "left" }}>Category</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={12}>
            ...
            <Grid item xs={12} sm={12} md={12} lg={12} justifyContent={"left"}>
              <Typography color="#646c9a" fontSize="1.2rem" sx={{ mt: "5px", mr: "15px", textAlign: "left" }}>Short Description</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={12}>
              <OutlinedInput placeholder='Provide short description' fullWidth sx={{ height: '40px' }} value={shortDescription} onChange={(e) => { setshortDescription(e.target.value) }} />
            </Grid>
            ...
            <Grid item container lg={12} xs={12}  >
              <Grid mb={2} item container >
                <Grid item lg={3} md={3} sm={12} xs={12} sx={{ display: "flex", justifyContent: { xs: "center", sm: "center", md: "flex-end", lg: "flex-end" } }}><Typography mr="14px" fontSize="1rem" color="#646c9a" sx={{ textAlign: { xs: "left", sm: "left", md: "right", lg: "right" } }}>Blog thumbnail (400 x 225)</Typography>
                </Grid>
