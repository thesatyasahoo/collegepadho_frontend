import {TextareaAutosize, TextField,Grid, Paper, Typography,Select,MenuItem, Button, Box, IconButton, Divider } from '@mui/material'
...
const fetchCourse = ()=>{
  axios.get(`${BASE_URL}/api/collegeinfo/${param.id}`).then((res)=>{setIsDataReceived(true);setIsData(res.data.response.collegeCourse)})
  }
...
useEffect(()=>{window.scrollTo(0,0)},[isCourse])
...
{isCourse === 'With Course'? <form onSubmit={dataToServer}>
    <Grid item container sx={{ justifyContent: "center", alignItems: "center" }} spacing={2} >
           <Typography m="20px" sx={{color:"green",fontSize:"30px",fontWeight:"800"}}><u>Total Courses Added ({isData.length})</u></Typography>
        ...
        <Grid   mb={2}  item container sx={{display: isCourse === "With Course"?'flex':'none',justifyContent: "center", alignItems: "center" }} spacing={2}>
            <Paper sx={{m:"10px",p:"5px",width:"100%",mb:"0px"}} elevation={0}>
                <Typography>New Course Form</Typography>
                 ...
                 <TextComponent  in={9}  onChange={(e,i)=>{const newList =[...dataForm];newList[i].tutionFees = e.target.value;setDataForm(newList)}} value={dataForm[9].tutionFees}  label="Total tution fees (Rs.)" placeholder = "Please enter only number" />
                  ...
                  <Grid item xs={12} sx={{display:"flex",justifyContent:"center"}}>
                     <Paper  sx={{border:"1px solid #ebedf2",width:"70%",height:"fit-content",display:"flex",alignItems:"center",p:"10px",flexDirection:"column"}} elevation={3}>
                         <img width="60%" height="60%" htmlFor="searchInputadmin" src={imageUrl} alt="empty"></img>
...
<form key={item._id} onSubmit={(e,id)=>dataToServer(e,item._id)}>
  <Divider sx={{border:"2px solid green"}}/>
    <Grid item container sx={{ justifyContent: "center", alignItems: "center" }} spacing={2} >
  <Typography m="20px" sx={{color:"green",fontSize:"30px",fontWeight:"800"}}><u>Filled Course ({index+1})</u></Typography>
        <Grid   mb={2}  item container sx={{display: isCourse === "With Course"?'flex':'none',justifyContent: "center", alignItems: "center" }} spacing={2}>
            <Paper sx={{m:"10px",p:"5px",width:"100%",mb:"0px"}} elevation={0}>
                <Typography>Add course</Typography>
                 <SelectComponent in={index} onChange={(e,index)=>{const newList =[...isData];newList[index].stream = e.target.value;setIsData(newList)}} value={item.stream} label="Stream" placeholder="Select Stream" listItems={data.map((ele)=>{return (ele.stream)})}/>
