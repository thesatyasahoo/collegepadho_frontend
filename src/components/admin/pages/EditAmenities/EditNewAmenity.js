...
const EditNewAmenity = () => {
const param = useParams();
const [isAmenity,setIsAmenity] = useState([]);
const navigate = useNavigate();
const [value, setValue] = useState("")
const [textField,setTextField] = useState({amenitytitle:""});
    const fetchAmenity =()=>{
      axios.get(`${BASE_URL}/api/amenity`).then((res)=>{setIsAmenity(res.data.response.filter((item)=>{return item._id === param.id}))}).catch((err)=>{console.log(err)});
    }
    useEffect(()=>fetchAmenity(),[])
    useEffect(()=>{if(isAmenity.length > 0){setTextField({amenitytitle:isAmenity[0].amenity});setValue(isAmenity[0].icon)}},[isAmenity])
const dataToServer = ()=>{
